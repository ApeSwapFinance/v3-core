...
import { TestApeSwapV3SwapPay } from '../typechain/TestApeSwapV3SwapPay'
...
import { TestApeSwapV3ReentrantCallee } from '../typechain/TestApeSwapV3ReentrantCallee'
...
describe('ApeSwapV3Pool', () => {
  ...
  let factory: ApeSwapV3Factory
  let pool: MockTimeApeSwapV3Pool

  let swapTarget: TestApeSwapV3Callee
  ...
  async function initializeAtZeroTick(pool: MockTimeApeSwapV3Pool): Promise<void> {
    await pool.initialize(encodePriceSqrt(1, 1))
  ...
  describe('#lock', () => {
    ...
    it('cannot reenter from swap callback', async () => {
      const reentrant = (await (
        await ethers.getContractFactory('TestApeSwapV3ReentrantCallee')
      ).deploy()) as TestApeSwapV3ReentrantCallee
  ...
  describe('swap underpayment tests', () => {
    let underpay: TestApeSwapV3SwapPay
    beforeEach('deploy swap test', async () => {
      const underpayFactory = await ethers.getContractFactory('TestApeSwapV3SwapPay')
      underpay = (await underpayFactory.deploy()) as TestApeSwapV3SwapPay
